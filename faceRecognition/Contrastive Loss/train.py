...
from net import siameseNet as siameseNet
import dataset
import numpy as np
import cv2
import os
...
if __name__ == '__main__':
    ...
    loss = siamese_net.loss(left_out,right_out,pair_label_placeholder,margin)
    ...
    with tf.Session() as sess:
        ...
        for epoch_num in range(total_epoch_num):
            ...
            for iters_num in range(cur_epoch_iters_num):
                ...
                # _train_loss, _learning_rate = sess.run([loss, learning_rate], feed_dict=train_dict)
                _train_loss = sess.run([loss], feed_dict=train_dict)
                _test_loss = sess.run([loss], feed_dict=test_dict)
