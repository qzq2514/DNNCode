...
class ResNeXtForDigit(object):
    ...
    def bottleneck_MultiBranch(self,inputs,bottleneck_width,stride):
        ...
        concat_bottleneck = tf.concat(branchs,axis=-1,name="concat")
    ...
    def bottleneck_GroupConv(self,inputs,bottleneck_output_channel,stride):
        ...
        for gooup_id in range(self.cardinality):
            with tf.variable_scope("group_{}".format(gooup_id)):
    ...
    def resNeXt_unit(self,inputs,unit_output_channel,scope_name):
        with tf.variable_scope(scope_name):
            ...
            bottleneck_1x1=slim.batch_norm(bottleneck_1x1,activation_fn=None,scope="final_bn")
            ...
            if stride==2:   #下采样时bottleneck输出和idendity mapping分支的特征图大小不一样
                ...
                _input=slim.convolution2d(downsample_input,num_outputs=unit_output_channel,
                                          kernel_size=1,stride=1,scope="match_depth")
    ...
    def inference(self, inputs):
        ...
        with slim.arg_scope(self.ResNeXt_arg_scope(is_training=self._is_training)):
            ...
            for block_id,unit_output_channel in enumerate([64,128,256]):
                for unit_id in range(3):
                    net=self.resNeXt_unit(net,unit_output_channel,"unit{}_{}".format(block_id,unit_id))
                    print("unit_id:",unit_id)
                print("net",net)
            net_global_pool = tf.reduce_mean(net, [1, 2], name="global_pool", keep_dims=True)

            fc = slim.convolution2d(net_global_pool, num_outputs=self.num_classes,
                                     kernel_size=1, activation_fn=None, normalizer_fn=None, scope="full_conv")

            logits = tf.squeeze(fc, [1, 2], name='SpatialSqueeze')
            print("logits:", logits)
    ...
    def ResNeXt_arg_scope(self,is_training,weight_decay=0.0001,batch_norm_decay=0.95,
