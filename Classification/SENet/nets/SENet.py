...
class SENet(object):
    ...
    def inference(self, inputs):
        with slim.arg_scope(self.SE_ResNeXt_arg_scope(is_training=self._is_training)):
            ...
            for unit_id in range(3):
                net = self.SE_ResNeXt_unit(net, 256, False,
                                        scope_name_format.format(1,unit_id))

            for unit_id in range(4):
                net = self.SE_ResNeXt_unit(net, 512, unit_id == 0,
                                        scope_name_format.format(2,unit_id))

            for unit_id in range(6):
                net = self.SE_ResNeXt_unit(net, 1024, unit_id == 0,
                                        scope_name_format.format(3,unit_id))

            for unit_id in range(3):
                net = self.SE_ResNeXt_unit(net, 2048, unit_id == 0,
    ...
    def SE_ResNeXt_arg_scope(self,is_training,weight_decay=0.0001,batch_norm_decay=0.997,
